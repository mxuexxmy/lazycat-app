...
public class AppServiceImpl implements AppService {
    ...
    private static class AppListV3Response {
        private List<AppServiceImpl.AppV3Item> items;
        private int page;
        private int size;
        private int total;
    }
    ...
    private static class AppV3Item {
        ...
        private AppServiceImpl.CreateUser create_user;
        private AppServiceImpl.Information information;
        private AppServiceImpl.Version version;
        private AppServiceImpl.Rating rating;
        private AppServiceImpl.Count count;
    }
    ...
    private static class Rating {
        ...
        private AppServiceImpl.Statistics statistics;
    }
    ...
    private static class ScoreResponse {
        ...
        private AppServiceImpl.ScoreData data;
    ...
    public List<Map<String, Object>> getDeveloperRanking() {
        ...
        return creatorIds.stream()
                .map(creatorId -> {
                    User userInfo = userRepository.findById(creatorId)
                            .orElse(new User());
    ...
    public void updateApps() {
        ...
        try {
            ...
            while (hasMore) {
                String url = LazyCatInterfaceInfo.APP_LIST_URL + "?category_ids=0&sort=version_updated_at.desc&page=" + page + "&size=" + size;
                ...
                try (Response response = httpClient.newCall(request).execute()) {
                    ...
                    AppServiceImpl.AppListV3Response appListResponse = objectMapper.readValue(responseBody, AppServiceImpl.AppListV3Response.class);
                    ...
                    if (appListResponse.items != null && !appListResponse.items.isEmpty()) {
                        ...
                        Map<String, App> existingAppMap = existingApps.stream()
                                .collect(Collectors.toMap(App::getPackageName, app -> app));
                        ...
                        for (AppServiceImpl.AppV3Item appV3Item : appListResponse.items) {
                            try {
                                // 转换AppV3Item为App实体
                                App app = convertAppV3ToApp(appV3Item);

                                App existingApp = existingAppMap.get(app.getPackageName());
...
//                                        }
                                        ...
                                        log.info("更新应用: {}", app.getPackageName());
                                    }
                                } else {
...
//                                    }
                                    ...
                                    appRepository.save(app);
                                    log.info("新增应用: {}", app.getPackageName());
                                }

                                // 同步评分信息（从新接口的rating中获取）
                                if (appV3Item.rating != null) {
                                    syncAppScoreFromV3(app.getPackageName(), appV3Item.rating);
                                }
                                ...
                                if (isInitialSync) {
                                    // 不再同步用户信息和GitHub信息
                                    // 同步评论信息
                                    syncAppComments(app.getPackageName());
        ...
        for (int i = 0; i < totalApps; i += batchSize) {
            ...
            for (App app : batch) {
                try {
                    ...
                    updatedApps.add(app);
                    successCount++;
                    log.info("应用 {} 下载量更新完成", app.getPackageName());
                } catch (IOException e) {
                    log.error("网络错误: 更新应用 {} 下载量失败", app.getPackageName(), e);
                    failureCount++;
                } catch (Exception e) {
                    log.error("更新应用 {} 下载量时发生错误", app.getPackageName(), e);
                    failureCount++;
                }
                processedApps++;
            }
    ...
    private void updateDownloadCount(App app) throws IOException {
        try {
            Request request = new Request.Builder()
                    .url(LazyCatInterfaceInfo.DOWNLOAD_COUNT_URL + "?pkgId=" + app.getPackageName())
                    .get()
                    .build();
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                String responseBody = response.body().string();
                AppServiceImpl.DownloadCountResponse downloadCountResponse = objectMapper.readValue(responseBody,
                        AppServiceImpl.DownloadCountResponse.class);

                if (downloadCountResponse.errorCode == 0 && downloadCountResponse.data != null) {
                    app.setDownloadCount(downloadCountResponse.data);
                    log.info("成功更新应用 {} 的下载量: {}", app.getPackageName(), downloadCountResponse.data);
                } else {
            ...
            log.error("更新应用 {} 下载量时发生未知错误", app.getPackageName(), e);
...
//        }
    ...
    public long getTotalAppsCount() {
        try {
            String url = LazyCatInterfaceInfo.APP_LIST_URL + "?category_ids=0&sort=version_updated_at.desc&page=0&size=1";
            Request request = new Request.Builder()
                    .url(url)
                    .get()
                    .build();
...
//            appMap.put("category", app.getCategory());
    ...
    public void fixPackageNames() {
        ...
        for (App app : apps) {
            if (app.getPackageName() == null) {
                ...
                appRepository.save(app);
                fixed++;
                log.info("修复应用包名: {}", app.getPackageName());
            }
        }
    ...
    public void syncAppScore(String pkgId) {
        try {
            Request request = new Request.Builder()
                    .url(LazyCatInterfaceInfo.SCORE_API_URL + pkgId)
                    .get()
                    .build();
            ...
            try (Response response = httpClient.newCall(request).execute()) {
                ...
                String responseBody = response.body().string();
                AppServiceImpl.ScoreResponse scoreResponse = objectMapper.readValue(responseBody, AppServiceImpl.ScoreResponse.class);
    ...
    public void syncAppComments(String pkgId) {
        ...
        while (hasNext) {
            try {
                Request request = new Request.Builder()
                        .url(LazyCatInterfaceInfo.COMMENT_LIST_URL + pkgId + "?page=" + page + "&limit=" + limit)
                        .get()
                        .build();
                ...
                try (Response response = httpClient.newCall(request).execute()) {
                    ...
                    String responseBody = response.body().string();
                    AppServiceImpl.CommentListResponse commentResponse = objectMapper.readValue(responseBody,
                            AppServiceImpl.CommentListResponse.class);
    ...
    private App convertAppV3ToApp(AppServiceImpl.AppV3Item appV3Item) {
        App app = new App();
     ...
     */
    private void syncAppScoreFromV3(String pkgId, AppServiceImpl.Rating rating) {
        try {
            AppScore appScore = appScoreRepository.findById(pkgId)
                    .orElse(new AppScore());
